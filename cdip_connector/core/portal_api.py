...
class PortalApi:
    def __init__(self):
        ...
        self.device_states_endpoint = (
            f"{cdip_settings.PORTAL_API_ENDPOINT}/devices/states"
        )
    ...
    async def get_access_token(self, session: ClientSession) -> OAuthToken:
        ...
        response = await session.post(
            self.oauth_token_url, data=payload, ssl=cdip_settings.CDIP_ADMIN_SSL_VERIFY
        )
    ...
    async def get_authorized_integrations(
        ...
        logger.debug(
            f"get_authorized_integrations for : {cdip_settings.KEYCLOAK_CLIENT_ID}"
        )
        ...
        logger.debug(
            f"Got {len(json_response)} integrations for {cdip_settings.KEYCLOAK_CLIENT_ID}"
        )
    ...
    async def get_bridge_integration(self, session: ClientSession, bridge_id: str):
        ...
        headers = await self.get_auth_header(session)
        response = await session.get(
            url=f"{cdip_settings.PORTAL_API_ENDPOINT}/integrations/bridges/{bridge_id}",
            headers=headers,
            ssl=cdip_settings.CDIP_ADMIN_SSL_VERIFY,
        )
        response.raise_for_status()
